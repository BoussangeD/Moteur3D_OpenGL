...
namespace M3D_ISICG
{
	void TriangleMeshModel::load( const std::string & p_name, const FilePath & p_filePath )
	{
		...
		sortMeshesByOpacity();	// tri par opacité.
		std::cout << "Done! "						//
				  << _meshes.size() << " meshes, "	//
	...
	void TriangleMeshModel::sortMeshesByOpacity()
	{
	...
	Material TriangleMeshModel::_loadMaterial( const aiMaterial * const p_mtl )
	{
		...
		{
			material._isOpaque = false; // l'objet n'est pas opaque
		}
	...
	Texture TriangleMeshModel::_loadTexture( const aiString & p_path, const std::string & p_type )
	{
		...
		if ( image.load( fullPath ) )
		{
			...
			double mipmapLevels = std::log2( std::max( image._width, image._height ) );
			glTextureStorage2D( texture._id, (int)mipmapLevels, internalFormat, image._width, image._height );
